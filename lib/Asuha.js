...
class Asuha extends Callable {
   ...
   */
  addHost (hostKey, options) {
    const Remote = Asuha.REMOTES[hostKey]
    if (Remote) {
      this._hosts.set(Remote, new Remote(options))
    }
    return this
  }
  ...
  removeHost (hostKey) {
    const Remote = Asuha.REMOTES[hostKey]
    if (Remote) {
      return this._hosts.delete(Remote)
    }
    return false
  }
...
Asuha.REMOTES = { bitbucket: BitBucket }
