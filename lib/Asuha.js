...
class Asuha {
  ...
  off (event, listener) {
    if (this.midware) {
      this.midware.off(event, listener)
    }
    return this
  }

  /**
   * Claim an existing repository and listen for its update.
   *
   * Only effective after #listen() is called.
   * @param {string} repoPath
   */
  claim (repoPath) {
    if (this.midware) {
      return this.midware.claim(repoPath)
    }
    return Promise.reject(new Error('Asuha server is not listening.'))
  }

  /**
   * Disclaim a repository
   * Only effective after #listen() is called.
   * @param {string} repoPath
   *//**
    * @overload
    ...
    */
  disclaim (remoteHost, fullname) {
    if (this.midware) {
      return this.midware.disclaim(remoteHost, fullname)
    }
    return Promise.reject(new Error('Asuha server is not listening.'))
  }
