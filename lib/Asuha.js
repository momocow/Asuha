...
class Asuha {
  ...
  listen (...args) {
    ...
    switch (this[S_TYPE]) {
      ...
      case Asuha.TYPE.HTTP:
        this.server = http.createServer(this.midware)
    }
  ...
  on (event, listener) {
    return this.midware.on(event, listener)
  }

  once (event, listener) {
    return this.midware.once(event, listener)
  }

  off (event, listener) {
    return typeof listener === 'function'
      ? this.midware.removeListener(event, listener) : this.midware.removeAllListeners(event)
  }
