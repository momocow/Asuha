...
class AsuhaMidware extends Callable {
  ...
  async __call__ (req, res, next) {
    ...
    for (const host of this.hosts) {
      ...
      if (!localRepo) {
        this.emit('error', new LocalRepoNotFoundError(dns, fullname))
        return
      }
      ...
      try {
        for (const action of this.options.actions) {
          ...
          this.emit('action.pre', { fullname, owner, event }, action)
          this.emit('action.post', { fullname, owner, event }, action, await doAction(action, localRepo, this.options.stdio))
        }
      } catch (err) {
        this.emit('error', err)
      }
      this.emit('actions.post', { fullname, owner, event }, this.options.actions)
      this.emit('done', { fullname, owner, event })
      return
    }
