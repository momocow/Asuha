...
class AsuhaMidware extends Callable {
  ...
  init () {
    ...
    Object.keys(this.repoMap).forEach((host) => {
      Object.keys(this.repoMap[host]).forEach((repo) => {
        this.emit('init', this.repoMap[host][repo], repo, host)
      })
    })
  ...
  async __call__ (req, res, next) {
    ...
    for (const host of this.hosts) {
      ...
      this.emit('actions.pre', repoMata, this.options.actions, localRepo)
      try {
        for (const action of this.options.actions) {
          this.emit('debug', '- Action: %o', action)
          this.emit('action.pre', repoMata, action, localRepo)
          this.emit('action.post', repoMata, action, localRepo, await doAction(action, localRepo, this.options.stdio))
        }
      } catch (err) {
        this.emit('error', err)
      }
      this.emit('actions.post', repoMata, this.options.actions, localRepo)
