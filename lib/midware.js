...
function scanRepo (dir, subdir) {
  ...
  sectionSplitter.lastIndex = 0 // reset regexp
...
function getMidware (options) {
  options = Object.assign({}, getMidware.DEFAULT_OPTIONS, options)
...
class AsuhaMidware extends Callable {
  ...
  async init () {
    if (this.options.autoScan) {
      const res = scanLocalRepos(this.options.cwd, { match: this.options.match })
      this.repoMap = res
    }
  ...
  async __call__ (req, res, next) {
    ...
    for (const host of this.hosts) {
      ...
      if (!localRepo) {
        this.emit('debug', 'Local repository was not found. (host=%s, fullname=%s)', dns, repoMata.fullname)
